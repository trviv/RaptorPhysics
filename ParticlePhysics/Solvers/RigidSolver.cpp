...
#define RIGID_SOLVER_KERNEL 1
#define RIGID_SOLVER_SET_DELTA_POSITION_KERNEL 2
...
void RigidSolver::solve()
{
  ...
  ComputeUtil::get(positionUtilId)->calculateSum(compute, particlesTemp[0].device(), count, true);
  ...
  ComputeUtil::get(matrix3x3UtilId)->calculateSum(compute, covarianceMatrix.device(), count, true);
...

  {
    ...
    kernels[RIGID_SOLVER_KERNEL].setArgs(buffers, 3);
    kernels[RIGID_SOLVER_KERNEL].setArg<uint>(&step, 3);
    kernels[RIGID_SOLVER_KERNEL].setArg<uint>(&rigidBodyCount, 4);
    compute->execute(kernels[RIGID_SOLVER_KERNEL], workgroupSize, workgroupCount);
  }
...

  {
    ...
    kernels[RIGID_SOLVER_SET_DELTA_POSITION_KERNEL].setArgs(buffers, 3);
    kernels[RIGID_SOLVER_SET_DELTA_POSITION_KERNEL].setArg<uint>(&count, 3);
    compute->execute(kernels[RIGID_SOLVER_SET_DELTA_POSITION_KERNEL], workgroupSize, workgroupCount);
  }
